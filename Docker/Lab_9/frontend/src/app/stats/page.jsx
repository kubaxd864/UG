...
export default function Stats() {
  ...
  const viewStats = useMemo(
    () => ({
      ...
      requestsCount: Number(stats?.requestsCount ?? 0),
      serverTime: stats?.serverTime ?? "N/A",
    }),
    [stats],
  );
  ...
  return (
    <section className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <article className="rounded-2xl border border-gray-300 bg-gray-950 p-5 shadow-sm">
          <h2 className="text-sm font-medium text-gray-500">Uptime (s)</h2>
          <p className="mt-2 text-4xl font-semibold tracking-tight">
            {viewStats.uptime.toFixed(2)}
          </p>
        </article>
        ...
        <article className="rounded-2xl border border-gray-300 bg-gray-950 p-5 shadow-sm">
          ...
          <p className="mt-2 text-4xl font-semibold tracking-tight">
            {viewStats.requestsCount}
          </p>
        </article>

        <article className="rounded-2xl border border-gray-300 bg-gray-950 p-5 shadow-sm">
          <h2 className="text-sm font-medium text-gray-500">Server Time</h2>
          <p className="mt-2 text-base font-semibold tracking-tight break-all">
            {viewStats.serverTime}
          </p>
        </article>
      </div>
