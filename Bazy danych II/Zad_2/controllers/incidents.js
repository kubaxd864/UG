...
const getAll = async (req, res) => {
  try {
    ...
    const incidents = await incidentsService.findAll({ level, status });
    res.json({
      data: incidents,
      meta: { status, level, count: incidents.length },
    });
  } catch (err) {
    handleError(err, res);
  }
...
const create = async (req, res) => {
  try {
    const { location, level, status } = req.body || {};
    if (!location || !level || !status)
      return res
        .status(400)
        .json({ error: "location, level and status are required" });
    const incident = await incidentsService.create({ location, level, status });
    res
      .status(201)
...
export default { getAll, getById, create, assignNewHero, closeIncident };
